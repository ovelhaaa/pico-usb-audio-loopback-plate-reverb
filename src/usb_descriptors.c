...
char const *string_desc_arr[] = {
    ...
    "Pico",                      // 1: Manufacturer
    "Pico Audio Loopback",       // 2: Product
    NULL,                        // 3: Serials will use unique ID if possible
    "Output",                    // 4: Audio Interface
    "Input",                     // 5: Audio Interface
};
...
static uint8_t current_bit_rate = 24;
...
size_t usb_audio_frame_num(size_t frame_size) {
    return frame_size / CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX /
           CFG_TUD_AUDIO_FUNC_1_FORMAT_1_N_BYTES_PER_SAMPLE_RX;
    ;
}
size_t usb_audio_frame_size(void) {
    return (size_t)((current_sample_rate / 1000) * CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX *
                    CFG_TUD_AUDIO_FUNC_1_FORMAT_1_N_BYTES_PER_SAMPLE_RX);
}
...
void tud_mount_cb(void) { led_set_blink_interval(BLINK_MOUNTED); }

void tud_umount_cb(void) { led_set_blink_interval(BLINK_NOT_MOUNTED); }
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request) {
    ...
    if (alt != 0) {
        current_bit_rate = CFG_TUD_AUDIO_FUNC_1_FORMAT_1_RESOLUTION_RX;
    }
    return true;
}
